...
import models.{FeedItem, User, JsonFormats}
 ...
 */
object DatabaseTemplate extends Controller with MongoController {
  /*
  ...
  def create = Action {
    val user = User(BSONObjectID.generate, 10,"first","last")
    // insert the user
   // val futureResult = collection.insert(user)
    // when the insert is performed, send a OK 200 result

    collection insert BSONDocument( "name" -> "NewName" , "Date" -> BSONDateTime(DateTime.now().getMillis + 60000) )
